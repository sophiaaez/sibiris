...
class facenetVAE(nn.Module):
    def __init__(self,layer_amount=6,channels=1,isize=512,z_dim=64,layer_size=128,extradense=False):
        ...
        if self.extradense:
            self.extra = nn.Linear(z_dim,z_dim)
...
def evalSet(filepath,network_path=None):
    ...
    for inputs in loader:
        inputs = inputs.float().cuda()
        outputs,mu,logvar = model(inputs)
        lost,mselost,kldlost  = loss_fn(outputs,inputs,mu,logvar)
        total_loss += lost.detach().cpu().item()
        total_mse += mselost.detach().cpu().item()
        total_kld += kldlost.detach().cpu().item()
    print("Total loss for testset is: " + str(total_loss) + " MSE loss: " + str(total_mld) + " KLD loss: " + str(total_kld))
...
def objective(trial):
    ...
    if (int(stop_epoch/10)-10) < len(validation_losses):
        final_loss = validation_losses[(stop_epoch/10)-10] #10 coz of patience = 10
