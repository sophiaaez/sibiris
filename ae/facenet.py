...
def trainNet(net, batch_size, n_epochs, learning_rate,train_set,val_loader):
    ...
    for epoch in range(n_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            #print(outputs.shape)
            loss_size = loss(outputs.float().cuda(), inputs.float().cuda()) # labels)
            loss_size.backward()
            optimizer.step()
            #return outputs
        ...
        for inputs in val_loader:
           ...
           #Forward pass
            val_outputs = net(inputs.float().cuda())
            val_loss_size = loss(val_outputs.float().cuda(), inputs.float().cuda()) # labels)
            total_val_loss += val_loss_size.data.item()
            image  =val_outputs[0,0].cpu().detach()
            io.imsave("test_images/set/outputs/output_e" + str(epoch) + ".jpg", (color.grey2rgb(image)*255).astype(np.uint8))
...
def doYaThang(path,epochs,load=""):
	...
	if load:
		...
		CNN = facenet()
	trainNet(CNN, batch_size=1, n_epochs=epochs, learning_rate=0.001,train_set=whales,val_loader=val_loader)
	torch.save(CNN, str("cnnsave"))



def showOutputs(path,number):
	CNN.torch.load(str(path + "cnnsave_"+number))
...
def main():
	...
	doYaThang(path="test_images/set/*.jpg",epochs=200)
