...
def getDatasets(filepath,validation_split=1/3,reduction=0.125):
    size = 512
    set_raw = []
...
def trainNet(epochs,learning_rate,batch_size,data_path,layers,layer_size,save=True):
    ...
    #EARLY STOPPER
    es = EarlyStopper(5,0.1,str("AE_earlystopsave_4_c_stn.pth"),save)
...
def getAndSaveEncodings(filepath,filename,network_path=None):
    ...
    with open(str('ae_' + name + '_encodings_c_stn.npy'), 'wb') as f:
        np.save(f, encodings)
    with open(str('ae_' + name + '_ids_c_stn.npy'),'wb') as f:
...
def matchTop10():
    model = torch.load("AE_earlystopsave_4_c_stn.pth").cuda()
...
if __name__ == "__main__":
    ...
    trainNet(epochs=1000,learning_rate=0.0001,batch_size=8,data_path="../data/trainingset_final.csv",layers=4,layer_size=64,save=True)
    getAndSaveEncodings("../data/trainingset_final.csv", "training","AE_earlystopsave_4_c_stn.pth")
    getAndSaveEncodings("../data/testset_final.csv","test","AE_earlystopsave_4_c_stn.pth")