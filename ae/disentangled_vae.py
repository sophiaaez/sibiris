...
def loss_fn(recon_x, x, mu, logvar,beta = 1):   # defining loss function for va-AE (loss= reconstruction loss + KLD (to analyse if we have normal distributon))
...
def getDatasets(path,batch_size,validation_split=0.33):
    size = 512
...
def trainNet(epochs,learning_rate,batch_size,beta=1):
    #DATA
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0: 
            ...
            print("Epoch " + str(epoch) + " with val loss " + str(total_val_loss))
            #EARLY STOPPING
            if len(validation_losses) > 1 and (validation_losses[-1] - validation_losses[-2]) > 1:
                print("TRAINING FINISHED AFTER " + str(epoch) + " EPOCHS. K BYE.")
                break
            else:
                torch.save(model,str("modelsave_5l_"+str(epoch)+"e")) #makes sure to save before validation loss increases
        training_losses.append(total_train_loss.detach().cpu().item()) 
    #SAVE LOSSES TO FILE
    filename = str("losses_5l.txt")
...
def main3():
    trainNet(epochs=300,learning_rate=0.0002,batch_size=16)
