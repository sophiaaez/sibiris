...
class SiameseNetwork(nn.Module):
    def __init__(self):
        ...
        self.l333 = nn.Conv2d(128, 128, 3, stride=2,padding=2) 

        self.o = nn.Conv2d(128,256,1,stride=1,padding=1)
        self.fl = Flatten()
        self.d = nn.Linear(6400,512)
    ...
    def forward_once(self, x):
          ...
          x3 = nn.ReLU()(self.l3(x2))
          #x = nn.ReLU()(x2 + x3)
          x = self.bn(x3)
          #print(x.size())

          x1 = nn.ReLU()(self.l11(x))
          x2 = nn.ReLU()(self.l22(x1))
          x3 = nn.ReLU()(self.l33(x2))
          #x = nn.ReLU()(x2 + x3)
          x = self.bn(x3)
          #print(x.size())

          x1 = nn.ReLU()(self.l111(x))
          x2 = nn.ReLU()(self.l222(x1))
          x3 = nn.ReLU()(self.l333(x2))
          #x = nn.ReLU()(x2 + x3)
          x = self.bnn(x3)
          #print(x.size())
...
def train(epochs,learning_rate,batch_size,set1,ids1):
    ...
    model.train()
    mse = nn.MSELoss()
    es = EarlyStopper(patience=50,delta=0.001,save_path="siamese_network_vae_ff",save=True)
...
def matchTop10():
    model = torch.load("siamese_network_vae_ff")
    tr_enc = np.load("../ae/vae_training_encodings.npy")
    tr_ids = np.load("../ae/vae_training_encoding_ids.npy")
