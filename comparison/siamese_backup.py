...
def train(epochs,learning_rate,batch_size,set1,ids1,size1=128,size2=32):
    ...
    model.train()
    mse = nn.MSELoss()
    es = EarlyStopper(patience=30,delta=0.1,save_path="siamese_network_vae_correct_v2_2.pth",save=True)
...
def matchTop10():
    model = torch.load("siamese_network_ae_correct_v2_2.pth").cuda()
    tr_enc = np.load("../ae/ae_training_encodings_simple_v2.npy")
    tr_ids = np.load("../ae/ae_training_ids_simple_v2.npy")
    te_enc = np.load("../ae/ae_test_encodings_simple_v2.npy")
    te_ids = np.load("../ae/ae_test_ids_simple_v2.npy")
    ...
    te_idx = randomSubset(1000,len(te_ids))
    te_enc = te_enc[te_idx]
    te_ids = te_ids[te_idx]
    pred = []
    tp = 0
