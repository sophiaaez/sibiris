...
def train(epochs,learning_rate,batch_size,set1,ids1):
    ...
    model.train()
    mse = nn.MSELoss()
    es = earlyStopping(patience=10,delta=0.01,save_path="siamese_network_ae_",save=True)
    for epoch in range(epochs):
        ...
        if stop:
          print("TRAINING FINISHED AFTER " + str(epoch-10) + " EPOCHS. K BYE.")
          break
    #torch.save(model,str("siamese_network"))
...
def match1():
    ...
    pred = []
    for i in range(len(te_ids)):
      te = torch.from_numpy(np.array([te_enc[i]])).cuda()
      matches = []
      for j in range(len(tr_ids)):
        tr = torch.from_numpy(np.array([tr_enc[j]]))
        res = model(te,tr)
        if res > 0.5: 
          matches.append([res.item(),tr_ids[j,-1]])
      matches = matches[matches[:,0].argsort()]
      for i in range(1,11):
        m_id = matches[-i,-1]
        if te_ids == m_id:
